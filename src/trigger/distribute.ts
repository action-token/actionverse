  ...
  run: async ({ id }: { id: string }) => {
    ...
    try {
      ...
      const eligibleUsers = parsedData
        ...
        .map((user) => ({
          pubkey: user.pubkey,
          amount: (user.plotBal * user.plots.length).toFixed(7), // Stellar precision
        }));
      ...
      console.log(
        `Total eligible users: ${eligibleUsers.length}, Already completed: ${completedUsers.length}`,
      );
      ...
      console.log(`Remaining users to distribute: ${remainingUsers.length}`);
