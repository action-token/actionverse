      ...
      .flatMap((group) => {
        ...
        if (multiPin) {
          return group.locations.map((location) => ({
            // `LocationGroup` has its own `latitude`/`longitude` (the rule
            // template's first point) — spread after `...group` so each
            // pin keeps its OWN coordinates instead of every pin in the
            // group collapsing onto the group's single representative
            // point.
            ...group,
            ...location,
            id: location.id,
