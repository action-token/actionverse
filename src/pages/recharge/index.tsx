        ...
        >
            <div className="flex flex-col items-center justify-center gap-6">
                ...
                >
                    ...
                    <h2 className="text-3xl font-bold">Recharge
                        <span className="text-primary"> {process.env.NEXT_PUBLIC_ASSET_CODE}</span> Tokens
            ...
            <div className="flex flex-col items-center justify-center gap-6">
                ...
                <Alert variant='destructive'>
                    ...
                    <AlertDescription className="text-sm text-muted-foreground">
                        The price of {process.env.NEXT_PUBLIC_ASSET_CODE} tokens may vary based on market
                        conditions. Please check the current price before proceeding with your purchase.
                    </AlertDescription>
                </Alert>
                <AnimatePresence>
                    {selectedOffer && (
                ...
                {session.status == "authenticated" && selectedOffer && (
                    ...
                    >
                        <Button
                            variant="default"
                            size="lg"
                            className="w-full md:w-2/3 py-6 text-base font-medium"
...
function SelectedOfferSummary({ offer }: ISelectedOfferSummary) {
    if (offer)
        return (
            <motion.div
                className="bg-muted/50 rounded-lg p-4 text-center w-full"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 0.2 }}
            >
                <p className="text-lg font-medium">
                    Buy <span className="text-primary font-bold">{offer.num.toFixed(2)}</span>{" "}
