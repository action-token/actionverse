...
import { MapPin, ScanLine, RefreshCcw, Crosshair, Zap, Trophy, Star } from "lucide-react"
...
export default function HomeScreen() {
    ...
    const getNearbyPins = (userLocation: UserLocationType, locations: ConsumedLocation[], radius: number) => {
        return locations.filter((location) => {
            if (location.auto_collect || location.collection_limit_remaining <= 0 || location.collected) return false
            const distance = getDistanceFromLatLonInMeters(userLocation.lat, userLocation.lng, location.lat, location.lng)
    ...
    const getAutoCollectPins = (userLocation: UserLocationType | null, locations: ConsumedLocation[], radius: number) => {
    ...
    useLayoutEffect(() => {
        const updateButtonLayouts = () => {
            ...
            const allElementsExist = welcome && balance && refreshButton && recenterButton && arButton
    ...
    if (!locationPermissionGranted) {
        return (
            <LocationPermissionHandler onLocationGranted={handleLocationGranted} onLocationDenied={handleLocationDenied} />
        )
    }
            ...
            {locations.map((location: ConsumedLocation, index: number) => (
                ...
                >
                    <div className="relative group cursor-pointer">
                        <div className="w-12 h-12 rounded-full border-3 border-white shadow-xl overflow-hidden bg-white hover:scale-110 transition-transform duration-200">
                            <Image
                                height={48}
                                width={48}
                                alt="Pin"
                        ...
                        {location.collection_limit_remaining > 0 && !location.collected && (
                            <div className="absolute -top-1 -right-1 w-5 h-5 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-full flex items-center justify-center border-2 border-white shadow-lg">
                                <span className="text-xs font-bold text-white">{location.collection_limit_remaining}</span>
                            </div>
                        )}
                        <div className="absolute -inset-1 bg-white/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
                    </div>
                </Marker>
            ))}
        </>
