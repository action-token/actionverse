...
export function ModeSwitch() {
  ...
  return (
    <div
      className={`relative  w-full overflow-hidden rounded-full bg-gradient-to-r from-purple-500 to-pink-500 p-1 shadow-lg ${isMinimized ? "h-8" : " h-12"}`}
    >
      <Button
        className="absolute left-0 top-0 z-10 h-full w-full bg-transparent hover:bg-transparent"
        onClick={handleToggle}
      >
        <span className={`${isMinimized ? "hidden" : "block"}`}>
          Switch to {selectedMode === Mode.User ? "Creator" : "User"}
        </span>
      </Button>
      <motion.div
