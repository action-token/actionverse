...
import Asset from "../common/admin-asset";
  ...
  if (assets.data) {
    return (
      <>
        ...
        <div className="grid gap-4 overflow-y-auto scrollbar-hide sm:grid-cols-2 md:grid-cols-3   lg:grid-cols-4 xl:grid-cols-6">
          {assets.data?.pages.map((page, i) =>
            page.assets.map((item, j) => (
              <Asset key={`asset-${i}-${j}`} asset={item} />
            )),
          )}
          {assets.hasNextPage && (
            <Button
